...
public class Constants {
    ...
    public synchronized static JSONObject getLocation(AMapLocation location) throws JSONException {
        ...
        //errCode等于0代表定位成功，其他的为定位失败，具体的可以参照官网定位错误码说明
        if (location.getErrorCode() == 0) {
            int len = location.getProvince().length()+location.getCity().length()+location.getDistrict().length()+location.getStreet().length();
...
//            jsonObject.put("anotherName",location.get)
