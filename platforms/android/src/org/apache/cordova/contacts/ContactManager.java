    ...
    public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("search")) {
            if (Build.VERSION.SDK_INT >= 23) {
                if (ContextCompat.checkSelfPermission(this.cordova.getActivity(), READ) != PackageManager.PERMISSION_GRANTED
                        || ContextCompat.checkSelfPermission(this.cordova.getActivity(), WRITE) != PackageManager.PERMISSION_GRANTED) {
                    PermissionManage.openPermissionHint(cordovaPlugin, R.string.dialog_title_contact, R.string.request_contact_permission);
                } else {
