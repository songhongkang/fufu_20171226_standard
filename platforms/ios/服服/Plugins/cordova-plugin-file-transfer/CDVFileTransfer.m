...
@interface CDVFileTransfer () <ShowImageListDelegate,NSURLSessionDelegate,CDVAlertMessageViewDelegate>
...
- (void)uploadMore:(CDVInvokedUrlCommand *)command {
    _callID = command.callbackId;
    NSArray *urlArr = [[command argumentAtIndex:0] componentsSeparatedByString:@","];
    ...
    [self fileUploadWithServer:server fileKey:fileKey fileName:fileName url:urlArr options:options];
}

- (void)getAssetImage:(CDVInvokedUrlCommand *)command {
    
    _callID = command.callbackId;
    
    NSString *count = [command argumentAtIndex:0];
    
    ALAuthorizationStatus author = [ALAssetsLibrary authorizationStatus];
    if (author == ALAuthorizationStatusRestricted || author ==ALAuthorizationStatusDenied){
        //无权限 做一个友好的提示
        self.result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:nil];
        [self.commandDelegate sendPluginResult:self.result callbackId:self.callID];
        
//        UIAlertView * alart = [[UIAlertView alloc]initWithTitle:@"提示" message:@"请您设置允许\"服服\"访问您的相册\n设置>隐私>照片" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
//        [alart show];
...
//        [self.commandDelegate sendPluginResult:nil callbackId:self.callID];

//        self.result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:nil];
//        [self.commandDelegate sendPluginResult:self.result callbackId:self.callID];
     
    } else {
        ShowImageListController *showImage = [[ShowImageListController alloc] init];
        showImage.delegate = self;
        showImage.selectCount = [count intValue];
        CustomeNaigationViewController *nav = [[CustomeNaigationViewController alloc] initWithRootViewController:showImage];
        [self.viewController presentViewController:nav animated:YES completion:nil];
        
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(backImageUrl:) name:@"ReturImageUrl" object:nil];
    }
}
...
- (void)clearCacheImage:(CDVInvokedUrlCommand *)command {
    // 获取tmp目录
    NSString *tmpPath = NSTemporaryDirectory();
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error = nil;
    NSArray *tempArray = [fileManager contentsOfDirectoryAtPath:tmpPath error:nil];
...
- (void)imageBrowse:(CDVInvokedUrlCommand *)command {
    ImageBrowseViewController *imageBrowse = [[ImageBrowseViewController alloc] init];

    CustomeNaigationViewController *nav = [[CustomeNaigationViewController alloc] initWithRootViewController:imageBrowse];
    [self.viewController presentViewController:nav animated:YES completion:nil];
}

- (void)backImageUrl:(NSNotification*)notification {
    ...
    if (path != nil && path.length > 0) {
        uploadPath = [self writeImageToTmpWithPath:path];
    }
    if (uploadPath.length > 0) {
        self.result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:uploadPath];
        [self.commandDelegate sendPluginResult:self.result callbackId:self.callID];
        
    }
    
    //    NSArray *arr = [path componentsSeparatedByString:@","];
    //    [self getImageWithUrl:arr];
    
    [[NSNotificationCenter defaultCenter] removeObserver:@"ReturImageUrl"];
    
}

//将相册中图片写入到tmp目录中
- (NSString *)writeImageToTmpWithPath:(NSString *)path {
    ...
    for (int i = 0; i < pathArr.count; i++) {
        ...
        NSString *imageTmpPath = [tmpPath stringByAppendingPathComponent:[NSString stringWithFormat:@"cdv_photo_moreupload_00%.6d.jpg",num]];
        
        if ([fm fileExistsAtPath:imageTmpPath]) {
            int num = (arc4random() % 1000000);
            imageTmpPath = [tmpPath stringByAppendingPathComponent:[NSString stringWithFormat:@"cdv_photo_moreupload_00%.6d.jpg",num]];
        }
        if (i == 0 || i == pathArr.count) {
            uploadPath = [uploadPath stringByAppendingFormat:@"%@",imageTmpPath];
        } else {
            uploadPath = [uploadPath stringByAppendingFormat:@",%@",imageTmpPath];
        }
        UIImage *image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:pathArr[i]]]];
        UIImage *compressImage = [self compressImage:image toTargetWidth:1280];
        NSData *data = UIImageJPEGRepresentation(compressImage, 0.6);
...
- (void)fileUploadWithServer:(NSString *)server fileKey:(NSString *)fileKey fileName:(NSString *)fileName url:(NSArray *)urlArr options:(NSDictionary *)options{
    NSArray *imageArr = [self getImageWithUrl:urlArr];
    [self uploadImageWithFileKey:fileKey FileName:fileName Images:imageArr server:server options:options];
    
}

- (void)uploadImageWithFileKey:fileKey FileName:(NSString *)name Images:(NSArray *)images server:(NSString *)server options:(NSDictionary *)options{
    // 1. 确定请求路径
    NSURL *url = [NSURL URLWithString:server];
    // 2. 创建可以变的请求对象
    ...
    // 4. 设置请求头部信息, 上传请求
    NSString *header =  [NSString stringWithFormat:@"multipart/form-data; boundary=%@",kFormBoundary];
    ...
    for (NSString *key in options) {
        ...
        [mdata appendData:formBoundaryData];
        [mdata appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", key] dataUsingEncoding:NSUTF8StringEncoding]];
        [mdata appendData:[val dataUsingEncoding:NSUTF8StringEncoding]];
        [mdata appendData:[@"\r\n" dataUsingEncoding : NSUTF8StringEncoding]];
    }
    
    for (int a = 0; a < images.count; a++) {
        NSData *data = [self getBodyDataWithImage:images[a] fileKey:[NSString stringWithFormat:@"%@_%d",fileKey,a]name:name];
        [mdata appendData:data];
    }
    
    [mdata appendData:[[NSString stringWithFormat:@"--%@",kFormBoundary] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    /*
     ...
     NSError：请求的错误信息
     */
    [[self.session uploadTaskWithRequest:request fromData:mdata completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        // 8. 解析数据
        NSLog(@"datat=====%@",[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding]);
        self.result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding]];
        [self.commandDelegate sendPluginResult:self.result callbackId:self.callID];
    }]
...
// 拼接上传的数据
- (NSData *)getBodyDataWithImage:(UIImage *)image fileKey:(NSString *)fileKey name:(NSString *)name {
    ...
    /*
     ...
     文件数据
     */
    [data appendData:[[NSString stringWithFormat:@"--%@",kFormBoundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [data appendData:KnewLine];
    [data appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"; filename=\"%@\"",fileKey,name] dataUsingEncoding:NSUTF8StringEncoding]];
    [data appendData:KnewLine];
...
//从tmp目录中获取图片保存到数组中
- (NSArray *)getImageWithUrl:(NSArray *)strArr {
    NSMutableArray *imageArr = [[NSMutableArray alloc] init];
    for (NSString *str in strArr) {
        UIImage *image = nil;
        if ([[str stringByReplacingOccurrencesOfString:@"file://" withString:@"/private"] rangeOfString:@"?dt"].length != 0) {
            NSRange range = [[str stringByReplacingOccurrencesOfString:@"file://" withString:@"/private"] rangeOfString:@"?dt"];
            image = [UIImage imageWithContentsOfFile:[[str stringByReplacingOccurrencesOfString:@"file://" withString:@"/private"] substringToIndex:range.location]];
        } else {
            image = [UIImage imageWithContentsOfFile:[str stringByReplacingOccurrencesOfString:@"file://" withString:@"/private"]];
        }
        if (image != nil) {
...
- (void)retunSelectImage:(NSString *)imageUrl imageArr:(NSArray *)imageArr {
    ...
    if (imageUrl.length > 0 && imageUrl != nil) {
        uploadPath = [self writeImageToTmpWithPath:imageUrl];
    }
