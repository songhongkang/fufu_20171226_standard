...
-(void)initPlugin{
    ...
    if (app.hk_message) {
        [JPushPlugin fireDocumentEvent:JPushDocumentEvent_OpenNotification jsString:app.hk_message];
    }
...
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"推送设置" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:@"Cancel", nil];
//        [alert show];
...
+ (void)hk_monitorBlueToothStates:(int)states withSn:(NSString *)sn
{
//    NSLog(@"states:%d,sn:%@",states,sn);
    if (sn == nil) {
        sn = @"";
    }
    NSDictionary *dic = @{
                          @"sn":sn,
                          @"status":[NSNumber numberWithInt:states],
                          @"time":[NSString currentTimeStr],
                          @"name":@"11"
                          };
    MyNSLog(@"dic:%@",dic);
...
//    str =  [dic toJsonString];
    NSString *stringKey =  [NSString stringWithFormat:@"cordova.fireDocumentEvent('bluetooth.receiveStatus',%@)",str];
//    NSLog(@"str --->%@",stringKey);
    
    [SharedJPushPlugin.commandDelegate evalJs:stringKey];
}

+ (void)monitorBlueInRegion:(NSString *)sn withInside:(NSString *)isInisde
{
    NSLog(@"sn--->%@",sn);
    ...
    NSString *str =  [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
   str =  [dic toJsonString];
   NSString *stringKey =  [NSString stringWithFormat:@"cordova.fireDocumentEvent('signin.autoSignIn',%@)",str];
    NSLog(@"str --->%@",str);
    [SharedJPushPlugin.commandDelegate evalJs:stringKey];
}
...
- (void)networkDidReceiveMessage:(NSNotification *)notification {
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Title" message:@"init notification" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
